...
class GoalQuery:
    ...
    def load_matching(
        ...
        goal_dicts = [
            goal_dict for goal_dict in
            self._collection.find(query)
        ]

        for i in range(len(goal_dicts)):
            goal_dicts[i]['_id'] = str(goal_dicts[i]['_id'])
            goal_dicts[i]['metrics'] =\
                self._metric_query.load_related_metrics(goal_dicts[i])
    ...
    def _filter_goals_by_status(
        ...
        match status:
            case 'in_progress':
                return list(filter(
                    lambda goal:
                    self._goal_factory.from_dict(goal).in_progress(),
                    goals
                ))
            case 'failed':
                return list(filter(
                    lambda goal:
                    self._goal_factory.from_dict(goal).was_failed(),
                    goals
                ))
            case 'completed':
                return list(filter(
                    lambda goal:
                    self._goal_factory.from_dict(goal).was_completed(),
                    goals
                ))
            case _:
                return goals
